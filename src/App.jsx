...
// ================================================================
// AUTO-BLOCK — si tiene deuda sin pagar > 30 días → bloquear
// ================================================================
    ...
    <div className="modal-overlay">
      <div className="card" style={{ width:"100%", maxWidth:460, padding:32 }}>
        ...
        <p style={{ fontSize:13, color:"#71717A", lineHeight:1.65, marginBottom:20 }}>
          El chatbot usa <b style={{ color:"#FAFAFA" }}>GPT-4o</b>. También puedes configurarla en <code style={{ background:"#27272A", padding:"2px 6px", borderRadius:4 }}>.env</code> como <code style={{ background:"#27272A", padding:"2px 6px", borderRadius:4 }}>VITE_OPENAI_API_KEY</code>.
        </p>
        ...
        <p style={{ fontSize:11, color:"#3F3F46", marginBottom:20 }}>Solo persiste en memoria esta sesión. Se borra al recargar si no está en el .env.</p>
        ...
        <p style={{ fontSize:12, color:"#52525B" }}>
          Obtén tu key en <a href="https://platform.openai.com/api-keys" target="_blank" rel="noreferrer" style={{ color:"#F97316" }}>platform.openai.com/api-keys</a>
        </p>
      </div>
    </div>
  );
      ...
      <div className="page" style={{ maxWidth:700 }}>
        ...
        <div className="card" style={{ padding:22, marginBottom:18 }}>
          ...
          <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
            ...
            <button className="btn-ghost btn-sm" onClick={()=>audRef.current.click()}>🎙️ {audio?audio.name:"Subir audio"}</button>
          </div>
      ...
      <div style={{ flex:1, overflow:"hidden", display:"flex", maxWidth:980, margin:"0 auto", width:"100%", padding:"16px 24px", gap:18 }}>
        ...
        <div style={{ width:210, display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div className="card" style={{ padding:16 }}>
            <div style={{ fontSize:12, color:"#71717A", lineHeight:1.55, marginBottom:12 }}>💡 Puedes adjuntar imágenes o un audio describiendo tu pedido.</div>
            ...
            <button className="btn-ghost" style={{ width:"100%", fontSize:12 }} onClick={()=>audRef.current?.click()}>
            ...
            {orders.map(order => {
              ...
              return (
                <div key={order.id} className="card" style={{ padding:24, borderLeft:`3px solid ${svc?.accent||"#F97316"}` }}>
                  ...
                  {order.images?.length>0 && <div style={{ fontSize:12, color:"#52525B", marginBottom:10 }}>📎 {order.images.join(", ")}</div>}
                  {myQ.length>0 ? (
                    <div>
        ...
        {tab==="orders" && (
          ...
          : openOrders.length===0 ? (
            <div className="card" style={{ padding:52, textAlign:"center", color:"#71717A" }}>
              <div style={{ fontSize:40, marginBottom:12 }}>📭</div>No hay pedidos disponibles aún
            </div>
          ) : (
              ...
              {openOrders.map(order => {
                ...
                return (
                  <div key={order.id} className="card" style={{ padding:24, borderLeft:`3px solid ${svc?.accent||"#F97316"}` }}>
                    ...
                    <div style={{ display:"flex", gap:14, fontSize:13, color:"#71717A", marginBottom:14 }}>
                      {order.brief?.cantidad&&<span>📦 {order.brief.cantidad}</span>}
                      {order.brief?.plazo_estimado&&<span>📅 {order.brief.plazo_estimado}</span>}
                      {order.images?.length>0&&<span>📎 {order.images.length} imagen(es)</span>}
                    </div>
                    {alreadySent ? (
          ...
          myQuotes.length===0
            ...
            : myQuotes.map(q => (
              <div key={q.id} className="card" style={{ padding:20, marginBottom:12 }}>
                ...
                {q.status!=="closed" && (
                  <button className="btn-ghost btn-sm" onClick={()=>{ const order=openOrders.find(o=>o.id===q.orderId); if(order){ ctx.setActiveChat({ chatId:`${q.orderId}_${q.tallerId}`, order, quote:q }); ctx.setView("chat"); } else ctx.showToast("El pedido ya fue cerrado","err"); }}>
                    💬 Chat con el cliente
                  </button>
                )}
    ...
    <div>
      ...
      <div className="page" style={{ maxWidth:700 }}>
        <div className="card" style={{ padding:26, marginBottom:20, borderLeft:`3px solid ${svc?.accent||"#F97316"}` }}>
          ...
          <div style={{ display:"flex", gap:16, fontSize:13, color:"#71717A" }}>
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100vh" }}>
      ...
      <div style={{ flex:1, overflow:"hidden", display:"flex", flexDirection:"column", maxWidth:720, margin:"0 auto", width:"100%", padding:"16px 24px", gap:14 }}>
        <div className="card" style={{ padding:18, flexShrink:0 }}>
          <div style={{ fontWeight:800, fontSize:15, marginBottom:8 }}>{chat.order?.brief?.titulo||chat.order?.serviceName}</div>
          <div style={{ display:"flex", flexWrap:"wrap", gap:14, fontSize:13, color:"#A1A1AA", marginBottom:10 }}>
            <span>💰 S/ {chat.quote?.price}</span>
            <span>📍 {chat.quote?.location}</span>
            <span>📅 {chat.quote?.deliveryDate}</span>
          </div>
          {chat.quote?.status==="closed" && (
            <span style={{ fontSize:13, color:"#4ade80", fontWeight:700 }}>✓ Trato cerrado por S/ {chat.quote?.dealAmount}</span>
          )}
        </div>
        ...
        {loading ? (
          ...
          <>
            ...
            {tab==="users" && (
              <div className="card scroll-x">
                <table>
                  ...
                  <tbody>
                    {users.length===0 ? <tr><td colSpan={4} style={{ textAlign:"center", color:"#52525B", padding:28 }}>Sin usuarios</td></tr>
                    : users.map(u=>(
                      <tr key={u.id}>
            ...
            {tab==="talleres" && (
              <div className="card scroll-x">
                <table>
                  ...
                  <tbody>
                    {talleres.length===0 ? <tr><td colSpan={7} style={{ textAlign:"center", color:"#52525B", padding:28 }}>Sin talleres</td></tr>
                    : talleres.map(t=>{
            ...
            {tab==="orders" && (
              <div className="card scroll-x">
                <table>
                  <thead><tr><th>Cliente</th><th>Servicio</th><th>Brief</th><th>Estado</th><th>Fecha</th></tr></thead>
                  <tbody>
                    {orders.length===0 ? <tr><td colSpan={5} style={{ textAlign:"center", color:"#52525B", padding:28 }}>Sin pedidos</td></tr>
                    : orders.map(o=>(
                      <tr key={o.id}>
                        <td>{o.userName}</td>
                        <td><span className="tag">{o.serviceName}</span></td>
                        <td style={{ maxWidth:240, color:"#A1A1AA", fontSize:12 }}>{o.brief?.titulo}</td>
            ...
            {tab==="commissions" && (
              <div className="card scroll-x">
                <table>
                  ...
                  <tbody>
                    {closedQuotes.length===0 ? <tr><td colSpan={5} style={{ textAlign:"center", color:"#52525B", padding:28 }}>Sin tratos cerrados</td></tr>
                    : closedQuotes.map(q=>{
